/*
 * suite.shared.js — the full loop-police test suite, written against an
 * injected test adapter so one copy runs everywhere:
 *   - Node:    test.mjs calls registerSuite with node:test + node:assert/strict
 *   - Browser: playground.html calls runSuiteWithDefaults (mini-runner included
 *     here), loaded as a plain <script src> — works over file://
 *
 * The suite always runs against DEFAULTS: fixtures are calibrated to the
 * default thresholds (fixture lengths vs THINKING_WINDOW/OUTPUT_WINDOW/
 * PARA_MIN_LEN, semantic threshold 3, ...), so running it with a custom cfg
 * would measure the config, not the logic.
 */
(() => {
  "use strict";

  function registerSuite({ describe, test, assert }, L) {
    const {
      DEFAULTS, zArray, detectRepeatingSuffix, newSemanticState, detectSemanticLoop,
      ...
      hashToolCall, fmt, withSuffix, setConfigValue,
      migrateToolLoopBan, migrateRenamedKeys, splitHookCmd, buildDetectionPayload,
    } = L;
    const { THINKING_WINDOW, OUTPUT_WINDOW, MAX_WINDOW, PARA_MIN_LEN } = DEFAULTS;
    ...
    describe("fixtures", () => {
      test("A/B/C must be > THINKING_WINDOW", () => {
      ...
      test("P1..P4 must be > PARA_MIN_LEN", () => {
        assert.ok([P1, P2, P3, P4].every((p) => p.length > PARA_MIN_LEN));
      });
    });
    ...
    describe("output text loop detection (detectRepeatingSuffix with OUTPUT_WINDOW)", () => {
      ...
      test("fixture PHRASE must exceed OUTPUT_WINDOW", () => {
  ...
  // Browser-side runner: registers the suite into local arrays, runs everything
  // with cfg reset to DEFAULTS (like node --test), restores cfg, and returns
  // [{ name, tests: [{ name, pass, error? }] }] for the playground to render.
  function runSuiteWithDefaults(L) {
    const suites = [];
    let current = null;
    ...
    const assert = {
      ok(v, m) { if (!v) throw new AssertionError(m || `esperado truthy, recibido ${JSON.stringify(v)}`); },
      equal(a, b, m) { if (a !== b) throw new AssertionError(m || `${JSON.stringify(a)} !== ${JSON.stringify(b)}`); },
      notEqual(a, b, m) { if (a === b) throw new AssertionError(m || `esperado distinto de ${JSON.stringify(b)}`); },
      deepEqual(a, b, m) {
        if (L.stableStringify(a) !== L.stableStringify(b))
          throw new AssertionError(m || `${L.stableStringify(a)} !== ${L.stableStringify(b)}`);
      },
    };

    registerSuite({ describe, test, assert }, L);

    const saved = { ...L.cfg };
    Object.assign(L.cfg, L.DEFAULTS);
    const results = [];
    try {
      for (const suite of suites) {
        const r = { name: suite.name, tests: [] };
        results.push(r);
        for (const t of suite.tests) {
          try { t.fn(); r.tests.push({ name: t.name, pass: true }); }
          catch (e) { r.tests.push({ name: t.name, pass: false, error: String(e.message ?? e) }); }
        }
      }
    } finally { Object.assign(L.cfg, saved); }
    return results;
  }

  const api = { registerSuite, runSuiteWithDefaults };

  if (typeof module !== "undefined" && module.exports) module.exports = api;
  else globalThis.LoopPoliceSuite = api;
})();
